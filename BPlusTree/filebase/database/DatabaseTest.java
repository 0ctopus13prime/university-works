...
import fakeecommerce.database.config.Catalog;
import fakeecommerce.domain.Order;
import fakeecommerce.database.table.BplustreeFileSystemTableManager;
import fakeecommerce.database.table.TableManager;
import fakeecommerce.database.table.row.*;
...
public class DatabaseTest {
	...
	public void basicTest() {
		...
		fakeSpring.assemble("fakeecommerce/database");
	...
	public void translatorTest() {
		...
		assertThat(got.getbuzzId(), is("coupangId"));
	...
	public void validSampleOrder(Order order, long sampleOrderId) {
		assertThat(order.getbuzzId(), is("coupangId" + sampleOrderId));
	...
	public Order getSampleOrder() {
		Order order = new Order();
		order.setbuzzId("coupangId" + sampleOrderId);
		order.setDeliverStatus(Order.DeliverStatus.END);
	...
	public void duplicateInsertTest() {
		...
		for(int i = 0 ; i<userCount ; i++ ) {
			...
			for( Order got : orders ) {
				assertThat(got.getbuzzId(), is("coupangId" + i));
				assertThat(got.getDeliverStatus(), is(Order.DeliverStatus.END));
