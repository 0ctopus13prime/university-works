...
import fakecoupangsystem.exception.IllegalbuzzArgumentException;
 ...
 */
@Manager(FakebuzzConfig.class)
public class FakebuzzSystem {
	@Autowired
	...
	private FakebuzzConfig config;
	private Options options;
	private CommandLineParser parser;
	private HelpFormatter helpFormatter;

	public FakebuzzSystem() { }
	...
	public void launch() throws IOException {
		...
		helpFormatter.printHelp("buzz The World! Welcome!!", options);
	...
	private long order(CommandLine cmdLine) {
		...
		for(int i = 0 ; i<optionValues.length - 2 ; i++ ) {
			...
			if( productId < 0 || productId > 99 ) {
				throw new IllegalbuzzArgumentException("Invalid Product Id for [" +
						productId +
	...
	private void lookup(CommandLine cmdLine) {
		...
		if( ( value = properties.getProperty("order_id") ) != null ) {
			...
			orderDtos = lookupService.lookupBybuzzId(value);
		} else {
			throw new IllegalArgumentException();
		}
	...
	@Meta
	public void setConfig(FakebuzzConfig config) {
		this.config = config;
	}
}
